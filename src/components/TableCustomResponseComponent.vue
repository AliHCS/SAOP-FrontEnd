  ...
  <div id="data-table-component" :class="prefix">
    <DataTable class="table table-hover table-bordered" :options="OPTIONS">
      <thead>
        <tr>
          ...
          <th
            ...
            class="text-center action-header"
      ...
      <tbody>
        <tr
          ...
          :class="{ pointer: props.rowSelect }"
        >
          <td
            v-for="col in columns"
            :key="col.title"
            class="text-left truncate_text"
            :class="
              (checkField(getNestedField(value, col.data)),
              col.title.includes('Descripción') ? 'truncate_text' : '')
            "
            v-html="render(getNestedField(value, col.data))"
          ></td>
          <td
            v-if="!props.hideActions"
            class="text-center action_items"
            :class="{ 'fixed-actions-colum': props.fixedActions }"
          >
            <button
              v-if="showEdit"
            ...
            <button
              ...
              data-placement="top"
              title="Eliminar"
              @click="emit('onDelete', value[rowId])"
            >
              <i class="bi bi-trash"></i>
            </button>
            <button
              v-if="showDetail"
              class="btn btn-primary btn-sm active mt-2 mb-2"
              data-placement="top"
              title="Ver"
    ...
    >
      ...
      <ul class="pagination">
        <li
          class="page-item"
          :class="{ disabled: pagination.total_pages <= 1 }"
        >
          <a
            class="page-link"
            href="#"
...
//Funcion que obtiene los objetos de los campos de columns example: person.name
