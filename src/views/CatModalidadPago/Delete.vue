<template>
  <Transition name="view">
    <div v-if="showView">
      ...
      <hr class="red" />
      <CardConfirmComponent title="¿Estás Seguro?" subtitle="La modalidad de pago se dará de baja permanentemente."
      ...
      <form class="mt-4 card p-4">
        ...
        <div class="form-group">
          ...
          <input type="text" :value="data.descripcion_modalidad_pago" class="form-control" id="description-position"
            placeholder="Introduzca el nombre de la operación" autocomplete="off" disabled />
        </div>
      </form>
    </div>
...
const viewName = "Eliminar modalidad de pago";
const { getData, deleteData } = usePetition("cat_modalidad_pago/");
const itemId = ref("");
const showView = ref(false)
const data = reactive({
  clave: "",
  descripcion_modalidad_pago: "",
});

const handleCancel = () => router.push({ name: "listar-cat-modalidad-pago" });
const handleConfirm = () =>
