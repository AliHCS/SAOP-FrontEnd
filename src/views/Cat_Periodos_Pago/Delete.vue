<template>
    <Transition name="view">
      <div v-if="showView">
        ...
        <hr class="red" />
        <CardConfirmComponent title="¿Estás Seguro?" subtitle="El periodo de pago se dará de baja permanentemente."
        ...
        <form class="mt-4 card p-4">
          ...
          <div class="form-group">
            <label for="nombre-periodo-pago">Clave</label>
            <input type="text" :value="data.clave" class="form-control" id="clave"
              placeholder="" autocomplete="off" disabled />
          </div>
          <div class="form-group">
            <label for="nombre-periodo-pago">Descripción</label>
            <input type="text" :value="data.descripcion_periodo_pago" class="form-control" id="description"
              placeholder="" autocomplete="off" disabled />
          </div>
        </form>
      </div>
  ...
  const viewName = "Eliminar periodo de pago";
  const { getData, deleteData } = usePetition("cat_periodo_pago/");
  const itemId = ref("");
  const showView = ref(false)
  const data = reactive<ICatPeriodoPago>(defaultValues);

  
  const handleCancel = () => router.push({ name: "listar_periodos_pago" });
  const handleConfirm = () =>
