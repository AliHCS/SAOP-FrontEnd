<template>
    <Transition name="view">
      <div v-if="showView">
        ...
        <hr class="red" />
        <CardConfirmComponent title="¿Estás Seguro?" subtitle="El módulo se dará de baja permanentemente."
        ...
        <form class="mt-4 card p-4">
          ...
          <div class="form-group">
            ...
            <input type="text" :value="data.descripcion" class="form-control" id="description"
              placeholder="" autocomplete="off" disabled />
          </div>
        </form>
      </div>
  ...
  const viewName = "Eliminar módulo";
  const { getData, deleteData } = usePetition("cat_modulo/");
  const itemId = ref("");
  const showView = ref(false)
  const data = reactive<ICatModulo>(defaultValues);

  
  const handleCancel = () => router.push({ name: "listar_modulos" });
  const handleConfirm = () =>
