    ...
    <div class="container">
        <form role="form" @submit.prevent="saveData">
            <div class="row info-chip">
                <SelectComponent v-model="data.cartera_proyecto_inversion" title="No. Solicitud:" @change="fetchData()"
                    placeholder="Selecciona..." name="cartera_proyecto_inversion" id="cartera_proyecto_inversion"
                    class="col-sm-4" :options="arrayDataCartera.data" optionText="no_solicitud" />
                <i class="bi bi-info-circle-fill primary-red--text clickable" @click="infoNoSolicitud = true"></i>
            </div>
            <div class="row">
                <InputText :disabled="itemId !== ''" v-model="data.presupuesto_egresos_federacion" title="PEF:"
                    @change="fetchData()" placeholder="0" name="presupuesto_egresos_federacion"
                    id="presupuesto_egresos_federacion" class="col-sm-4" />
                <InputText title="&nbsp;" disabled placeholder="" class="col-sm-8"
                    v-model="presupuesto_egresos_federacion_desc" />
            </div>
            <div class="row">
                <SelectComponent v-model="data.pais" title="País:" placeholder="País" name="pais" id="pais" disabled
            ...
            <div class="row">
                <SelectComponent v-model="data.departamento" title="Departamento:" placeholder="Selecciona" name="pais"
                    disabled id="departamento" class="col-sm-12" :options="arrayDataDepto.data"
                    :optionText="'descripcion_corta'" />
            </div>
            <div class="row">
                <TextAraComponent v-model="data.descripcion" title="Descripción de los trabajos:"
                    :placeholder="'Descricpion...'" :name="'descripcion'" :id="'descripcion'" class="col-sm-12" />
            </div>
            <div class="row">
                <div class="col-sm-12 px-5 my-3">
                    <input type="checkbox" :checked="data.aplica_ruta_tramo_subtramo" class="form-check-input"
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    <input v-model="data.fecha_obra" type="date" class="form-control" id="fecha_obra" name="fecha_obra"
                        placeholder="" autocomplete="off" />
                </div>
                <SelectComponent v-model="data.estatus_trabajo" title="Estatus:" placeholder="Selecciona"
            ...
            <div class="row">
                <InputText :disabled="itemId !== ''" v-model.trim="data.meta" title="Meta:" placeholder="0.0"
                    name="meta" id="meta" class="col-sm-6" />
                <SelectComponent v-model="data.periodo" title="Periodo:" placeholder="Selecciona" name="periodo"
            ...
            <div class="row">
                <SelectComponent v-model="data.tipo_trabajo" title="Tipo trabajo:" placeholder="Selecciona"
                    name="tipo_trabajo" id="tipo_trabajo" class="col-sm-12" :options="arrayDataTrabajo.data"
                    :optionText="'descripcion'" />
            </div>
            <div class="row">
                <SelectComponent v-model="data.tipo_subtrabajo" title="Tipo subtrabajo:" placeholder="Selecciona"
                    name="tipo_subtrabajo" id="tipo_subtrabajo" class="col-sm-12" :options="arrayDataSubtrabajo.data"
                    :optionText="'descripcion'" optionValue="clave" />
            </div>
            <div class="row">
                <InputText :disabled="itemId !== ''" v-model.trim="data.actividad_prioritaria"
                    title="Actividad prioritaria:" placeholder="" name="actividad_prioritaria"
                    id="actividad_prioritaria" class="col-sm-4" />
                <InputText title="&nbsp;" disabled placeholder="" class="col-sm-8" />
            </div>
            <div class="row my-4">
                <div class="col-sm-4">
                ...
                <div class="col-sm-8">
                    <label class="form-check-label px-5">
                        <input type="radio" name="radio-01" value="unitario" class="form-check-input"
                            v-model="data.tipo_contratacion" />
                        <span class="pl-4">Precio unitario</span>
                    </label>
                    <label class="form-check-label px-5">
                        <input type="radio" name="radio-01" value="alzado" class="form-check-input"
                            v-model="data.tipo_contratacion" />
                        <span class="pl-4">Precio alzado</span>
                    </label>
                    <label class="form-check-label px-5">
                        <input type="radio" name="radio-01" value="mixto" class="form-check-input"
                            v-model="data.tipo_contratacion" />
            ...
            <div class="row">
                <TextAraComponent v-model="data.ubicacion" title="Ubicación:" :placeholder="'Descricpion...'"
                    :name="'ubicacion'" :id="'ubicacion'" class="col-sm-12" />
            </div>
            <div class="row">
                <TextAraComponent v-model="data.observaciones" title="Observaciones:" :placeholder="'Descricpion...'"
                    :name="'observaciones'" :id="'observaciones'" class="col-sm-12" />
            </div>
        ...
        <Modal v-if="infoNoSolicitud" :title="`Cartera de inversión`" saveButtonTitle="Aceptar" :large-modal="true"
            @onCloseModal="infoNoSolicitud = false" :showAcceptButton="false">
            <div class="card">
                <div class="card-body d-flex justify-content-center">
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr v-for="(cartera) in arrayDataCartera.data" @click="setCartera(cartera.id)"
                                class="clickable">
...
const { arrayData: arrayDataPEF, getDatas: getDatasPEF } = usePetition("presupuesto_egresos_federacion/");
...
async function saveData() {
    try {
        ...
        if (itemId.value) {
            ...
            let carteraAux = arrayDataCartera.value.data.find((cartera: any) => cartera.id == data.value.cartera_proyecto_inversion)
...
const fetchData = async () => {
    if (!data.value.cartera_proyecto_inversion || !data.value.presupuesto_egresos_federacion) return
    let baseData = arrayDataPEF.value.data.find((pef: any) => pef.cartera_proyecto_inversion == data.value.cartera_proyecto_inversion)
    let pais = arrayDataEntidadFederativa.value.data.find((entidad: any) => entidad.id == baseData.entidad_federativa)
    ...
    if (itemId.value) {
        getData(itemId.value)
            .then((response: any) => {
                ...
                data.value.tipo_subtrabajo = arrayDataSubtrabajo.value.data ? arrayDataSubtrabajo.value.data.find((subtrabajo: any) => subtrabajo.id == response.tipo_subtrabajo).clave : ''
